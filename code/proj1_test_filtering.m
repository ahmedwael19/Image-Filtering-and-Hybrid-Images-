...
%% Compare my FFT conv function with my time based filter with the imfilter

sobel_filter = [-1 0 1; -2 0 2; -1 0 1]; %should respond to horizontal gradients
tic;
sobel_image_t = my_imfilter(test_image, sobel_filter); %% my time based filter
subplot(1,3,1)
imshow(sobel_image_t + 0.5);
title("Sobel Filtered My Filter in Time domain");
imwrite(sobel_image_t + 0.5, 'sobel_image_t.jpg', 'quality', 95);
toc;

tic;
sobel_image_f = im_fft_conv(test_image, sobel_filter); %% my frequency based filter

subplot(1,3,2)
imshow(sobel_image_t + 0.5);
title("Sobel Filtered My Filter in Frequency domain");
imwrite(sobel_image_f + 0.5, 'sobel_image_f.jpg', 'quality', 95);
toc;

tic;
sobel_image_b = imfilter(test_image, sobel_filter); %% my frequency based filter

subplot(1,3,3)
imshow(sobel_image_b + 0.5);
title("Sobel Filtered MATLAB built-in function");
imwrite(sobel_image_b + 0.5, 'sobel_image_b.jpg', 'quality', 95);
toc;
